...
| ⭐ **我的自选** | 自选股一屏总览 | 迷你走势 + 最新价 / 涨跌幅，点击查看详细 K 线与各项指标 |
| 🎯 **批量回测** | 一套策略跑遍自选股，验证普适性 | 汇总盈利占比 / 跑赢基准占比 / 平均收益 / 回撤，附分标的明细 |
...
![回测看板](assets/image2.png)

看板包含 7 个页签：

| 页签 | 内容 |
| --- | --- |
| ⭐ **我的自选** | 自选股票列表：每行展示名称（下方小字代码）、**近 60 日迷你走势**、最新价、最新涨跌幅；点「详情」查看完整 K 线与 MACD / RSI / KDJ 各项指标曲线 |
| 🔬 **回测** | 选股票/策略/参数一键回测，展示指标卡片、多标的对比表、**K线 + 买卖点**、资金曲线、回撤曲线 |
| 🎯 **批量回测** | 选一套策略（侧栏策略或**我的策略库快照**），在自选股票上**批量回测**，汇总盈利占比 / 跑赢基准占比 / 平均收益 / 平均回撤 / 平均夏普等整体指标，并给出分标的明细与收益条形图，验证策略普适性 |
| 📊 **技术指标** | K线叠加均线/布林 + MACD / RSI / KDJ 子图，可查看原始指标数据 |
| 📚 **策略库** | **按策略分组**列出全部参数快照：折叠展开每种策略，快照按保存时间命名，点开看具体参数与保存时绩效；输入股票代码「应用回测」即可把该套策略/参数套到任意股票，结果直接推送到「🔬 回测」页展示；支持重命名 / 删除；底部保留多套策略同台**对比** |
